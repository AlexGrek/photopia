...
import type { Moodboard, MoodboardImage, MoodboardImageView, MoodboardSection, MoodboardSectionType } from '../Models';
...
const viewOptions: MoodboardImageView[] = ['grid', 'list', 'horizontalScroller'];
...
const EditMoodboardPage: React.FC = () => {
    ...
    const handleAddSection = (type: MoodboardSectionType) => {
        if (!moodboard) return;
        const newSection: MoodboardSection = type === 'text'
            ? { type: 'text', text: '' }
    ...
    const handleImageDescriptionChange = (sectionIndex: number, imageId: string, description: string) => {
        if (!moodboard) return;
        const sections = [...moodboard.sections];
    ...
    const handleRemoveImage = async (sectionIndex: number, image: MoodboardImage) => {
        if (!moodboard) return;
        ...
        try {
            const response = await fetch(`/api/v1/moodboardImage?moodboard_id=${moodboardId}&url=${encodeURIComponent(image.url)}`, {
                method: 'DELETE',
                headers: {
    ...
    const handleUploadImages = async (sectionIndex: number, files: FileList | null) => {
        ...
        try {
            for (const file of Array.from(files)) {
                ...
                const response = await fetch(`/api/v1/uploadMoodboardImage?moodboard_id=${moodboardId}`, {
                    method: 'POST',
                    headers: {
                        'X-Api-Key': apiKey,
                    },
                ...
                const uploaded = await response.json() as { id: string; url: string; width?: number; height?: number };
            ...
            notify(`Uploaded ${uploadedCount} image${uploadedCount === 1 ? '' : 's'}.`);
        } catch (err: unknown) {
            if (err instanceof Error) {
    ...
    const handleSave = async () => {
        ...
        setSaving(true);
        try {
            const response = await fetch(`/api/v1/updateMoodboard?moodboard_id=${moodboardId}`, {
                method: 'POST',
                headers: {
        ...
        <div className="bg-gray-950 text-white min-h-screen font-sans">
                ...
                <div className="absolute inset-0 bg-black/40 backdrop-blur-md flex flex-col justify-end p-6 md:p-10">
                    ...
                    <div className="flex flex-wrap items-center gap-4 mb-4">
                        ...
                        <input
                            ...
                            onChange={(e) => setMoodboard({ ...moodboard, name: e.target.value })}
                        />
                        <input
                            id='moodboard-color-inp'
                            type="color"
                            value={moodboard.headerColor}
                            onChange={(e) => setMoodboard({ ...moodboard, headerColor: e.target.value })}
            ...
            <main className="container mx-auto p-6 pt-10 fadeIn space-y-8">
                {moodboard.sections.map((section, index) => (
                    <div key={index} className="bg-gray-900 rounded-xl border border-gray-800 p-5 space-y-4">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2 text-gray-400 text-sm uppercase tracking-wide">
                                {section.type === 'text' ? <Type size={16} /> : <Images size={16} />}
                            ...
                            <div className="space-y-4">
                                <div className="flex items-center gap-3">
                                    ...
                                    <select
                                        ...
                                        onChange={(e) => handleSectionViewChange(index, e.target.value as MoodboardImageView)}
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                                    {(section.images || []).map((image) => (
                                        <div key={image.id} className="relative bg-gray-950 rounded-lg border border-gray-800 p-2 flex flex-col gap-2">
                                            ...
                                            <input
                                                ...
                                                onChange={(e) => handleImageDescriptionChange(index, image.id, e.target.value)}
